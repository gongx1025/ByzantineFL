...
# Dataset
def get_mnist_data(root_dir, train=True):
    mnist_stats = {
        "mean": (0.1307,),
        "std": (0.3081,),
    }
    transform = transforms.Compose(
        [
            transforms.ToTensor(),
            transforms.Normalize(mnist_stats['mean'], mnist_stats['std']),
        ]
    )
    dataset = datasets.MNIST(root=root_dir,
                             train=train,
                             download=True,
                             transform=transform)
    return dataset
...
def get_train_loader(root_dir, n_workers, alpha=1.0, batch_size=32, noniid=False):
    dataset = get_mnist_data(root_dir=root_dir, train=True)
    if not noniid:
...
def get_test_loader(root_dir, batch_size):
    dataset = get_mnist_data(root_dir=root_dir, train=False)
